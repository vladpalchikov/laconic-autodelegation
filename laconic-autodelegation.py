...
# constants
LACONIC_DECIMALS = 100000000000000000000
TRANSACTION_WAIT_TIME = 10

class LaconicAutodelegation():
    def __init__( self, config_file='config.ini' ):
        ...
        self.setup_telegram()
        self.setup_laconic_info()

        # send the hello message
        self.send( f'Hello from LACONIC Autodelegation Bot on {self.name}!' )
    ...
    def setup_laconic_info( self ):
        ...
        elif 'sleep_time' in self.config['LACONIC']:
            self.sleep_time = int(self.config['LACONIC']['sleep_time'])
        else:
            self.sleep_time = 600
        
        # bank reserve
        if "LACONIC_RESERVE" in os.environ:
            self.reserve = float(os.environ['LACONIC_RESERVE'])
        elif 'reserve' in self.config['LACONIC']:
            self.reserve = float(self.config['LACONIC']['reserve'])
        else:
            self.reserve = 0.1000

        # Prompt for the password if not in environment
        if "LACONIC_PASSWORD" in os.environ:
            self.password = os.environ['LACONIC_PASSWORD']
        elif 'password' in self.config['LACONIC']:
            self.password = self.config['LACONIC']['password']
        else:
        ...
        if "CHAIN_ID" in os.environ:
            ...
            self.chain_id = self.config['LACONIC']['chain_id']
        ...
        if "WALLET_NAME" in os.environ:
            ...
            self.wallet_name = self.config['LACONIC']['wallet_name']
        
        # wallet and validator keys
        if "WALLET_KEY" in os.environ:
            self.wallet_key = os.environ['WALLET_KEY']
        elif 'wallet_key' in self.config['LACONIC']:
            self.wallet_key = self.config['LACONIC']['wallet_key']
        elif "WALLET_ADDRESS" in os.environ:
            self.wallet_key = os.environ['WALLET_ADDRESS']
        elif 'wallet_address' in self.config['LACONIC']:
            self.wallet_key = self.config['LACONIC']['wallet_address']
        else:
        ...
        elif 'validator_key' in self.config['LACONIC']:
            self.validator_key = self.config['LACONIC']['validator_key']
        elif "VALIDATOR_ADDRESS" in os.environ:
            self.validator_key = os.environ['VALIDATOR_ADDRESS']
        elif 'validator_address' in self.config['LACONIC']:
            self.validator_key = self.config['LACONIC']['validator_address']
        else:
    ...
    def shares_to_decimal( self, shares ):
        ...
        '''
        return float( shares ) * ( 1/LACONIC_DECIMALS )

    def decimal_to_shares( self, amount ):
        '''
        return the decimal to shares conversion
        '''
        return int( amount * LACONIC_DECIMALS )

    def get_balance( self ):
        '''
        Obtain the LACONIC balance
        '''
        proc = Popen([ f"ethermintd q bank balances {self.wallet_key}" ], stdout=PIPE, shell=True)
    ...
    def get_delegations( self ):
        ...
        '''
        proc = Popen([ f"ethermintd q staking delegations-to {self.validator_key} --chain-id={self.chain_id}" ], stdout=PIPE, shell=True)
        (out, err) = proc.communicate()
...
# Create the object
laconic_bot = LaconicAutodelegation( args.config )

# run periodic delegation cycle
while True:
    laconic_bot.delegation_cycle()
